...
export class ToastService {
  private router = inject(Router);//为了点击跳转
  ...
  private createToastElement(message: string,alertClass: string,duration = 5000, avatar?:string,router?:string){
    ...
    if(router){
      // 添加点击跳转
      toast.addEventListener('click',() => this.router.navigateByUrl(router));
    }
